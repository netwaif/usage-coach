...
Claude · Codex는 단기(5시간 또는 일일)·7일 한도를 교차 분석해 5가지 상황으로 코칭하고, Antigravity는 7일(주간) 페이스만으로 코칭하며, 에이전트 루프용 **요금 가드**(`--hook` / `--guard-check`)를 함께 제공한다.

## 핵심 개념

- **단기(5시간 또는 일일) 한도** = 안 쓰면 reset으로 사라짐 → 남으면 아낌없이 소진(use-it-or-lose-it).
- **7일 한도** = 페이스 관리.
...
|--|--|--|
| 🔴 | 7일 한도가 빠듯 | 큰 작업은 미루기 |
| 🟢 | 단기(5h/일일) 넉넉 + 7일 여유 | 지금 큰 작업 |
| ⏳ | 단기(5h/일일) 부족 + 초기화 임박 | 곧 reset, 잠깐 대기 |
| 🟡 | 단기(5h/일일) 부족 + 페이스 뒤처짐 | 작은 작업만 |
| ⚪ | 평소 | 평소대로 |
...
- `coach --hook` — Claude Code Stop 훅용. 7일 가드. `coach guard on` 상태 + 루프 턴(`stop_hook_active`)일 때만 검사한다. 정지 시 `{"continue":false,"stopReason":"..."}`(사유 포함) 출력, 통과 시 무출력.
- `coach --guard-check` — provider 중립 체크. 정지 시 exit≠0 + 사유, 통과 시 exit 0.
- `coach guard on|off|status` — 런타임 스위치.

판정 = 하이브리드 OR: `left < timeLeft − m`(페이스) **또는** `left < floor`(절대). 데이터 오류 시 fail-open(통과). 설정 = `~/.config/usage-coach/guard.json` (provider별 `m`/`floor`/`pace`(페이스 게이트 on·off), 기본 0.10/0.10/`true`).
